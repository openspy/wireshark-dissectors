...
extern "C" {
        ...
        if (client_challenge != NULL) {
            ...
            if (server_challenge != NULL) {
                ...
                if (conv->challenge_setup == 0) {
                    ...
                    if (conv->game_info) {
                        gs_peerchat_init(&conv->client_ctx, (unsigned char *)conv->client_challenge, (unsigned char*)conv->game_info[2]);
                        gs_peerchat_init(&conv->server_ctx, (unsigned char*)conv->server_challenge, (unsigned char*)conv->game_info[2]);
                    }
                }
    ...
    int dissect_peerchat(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree _U_, void* data _U_) {
        ...
        if (conv->crypt_frame == 0) {
            ...
            if (crypt_bytes != NULL) {
                if (strncmp((const char *)crypt_bytes, "CRYPT", 5) == 0) {
