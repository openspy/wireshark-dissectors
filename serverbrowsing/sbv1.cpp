...
#define SBV1_VALIDATION_LEN 32
    ...
    int dissect_sbv1_server_challenge(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree _U_, void* data _U_) {
        ...
        if (conv->server_challenge == NULL) {
            conv->server_challenge = strdup((const char *)temp_str);
        }
    ...
    int dissect_sbv1_client_validation(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree _U_, void* data _U_) {
        ...
        const guint8* buff = tvb_get_ptr(tvb, offset, len);
        ...
        if (conv->client_validation == NULL) {
            conv->client_validation = strdup((const char *)temp_str);
        }

        char challenge_resp[90] = { 0 };
        gsseckey((unsigned char*)&challenge_resp, (unsigned char*)conv->server_challenge, (unsigned char*)conv->query_from_game[2], conv->enctype);
        ...
        if (strcmp(challenge_resp, (const char*)conv->client_validation) == 0) {
    ...
    int dissect_sbv1_query_response(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree _U_, void* data _U_) {
        ...
        if (conv->enctype != 0) {
            ...
            switch (conv->enctype) {
            case 1:
                dec_len = enctype1_wrapper((unsigned char *)conv->query_from_game[2], decrypted_heap_buffer, original_length);
                break;
            case 2:
                dec_len = enctype2_wrapper((unsigned char*)conv->query_from_game[2], decrypted_heap_buffer, original_length);
                break;
            }
    ...
    int dissect_sbv1_stream(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree _U_, void* data _U_) {
        ...
        const guint8* buff = tvb_get_ptr(tvb, 0, tvb_captured_length(tvb));
